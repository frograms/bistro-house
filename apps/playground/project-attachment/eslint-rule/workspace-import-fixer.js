...
function isInternalWorkspacePackage(packageName) {
  ...
  if (packageName.includes('eslint-config') || packageName.includes('prettier-config')) {
    return false;
  }
      ...
      if (fs.existsSync(dirPath) && fs.statSync(dirPath).isDirectory()) {
        ...
        for (const indexFile of indexFiles) {
          ...
          if (fs.existsSync(indexPath)) {
            ...
            const normalizedPath = relativePath.replace(/\\/g, '/').replace(/\.(ts|tsx|js|jsx)$/, '').replace(/\/index$/, '');
...
function findFileByExportName(dir, exportName, currentPath) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      const found = findFileByExportName(path.join(dir, entry.name), exportName, subPath);
...
const rule = {
  meta: {
    ...
    fixable: 'code',
    messages: {
      useDirectPath: 'workspace 패키지는 직접 경로로 임포트해야 합니다. {{suggestedPath}}',
    },
  },
  create(context) {
    return {
      ImportDeclaration(node) {
        ...
        const firstSpecifier = node.specifiers.find(s => s.type === 'ImportSpecifier');
        ...
        if (firstSpecifier) {
          ...
          if (directPath) {
            ...
            context.report({
              node: node.source,
              messageId: 'useDirectPath',
              data: { suggestedPath: `${source}/... (직접 경로를 지정해주세요)` },
            });
          }
        }
      },
