...
import { CommonPlaygroundPageToc } from "./view/_common/common-playground-page-toc";
      ...
      <div
        ...
        <main className={appContentCss.content} id="top">
          <header className={appContentCss.header}>
            ...
            <h1 id="overview">
              {activePage?.exampleLabel ?? "Playground"}
            </h1>

            {hasTableOfContents && activePage && (
              <p className={appContentCss.description}>
                {activePage.packageDescription}
              </p>
            )}

            {hasTableOfContents && activePage && (
              <div
                aria-label={`${activePage.packageName} 외부 링크`}
                className={appContentCss.pageMeta}>
                <a
                  className={appContentCss.pageMetaLink}
                  href={activePage.npmUrl}
                  rel="noreferrer"
                  target="_blank"
                >
                  <NpmIcon />
