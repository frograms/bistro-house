...
- 호스트와 **같은 인스턴스**를 써야 하는 경우 (React, ESLint 플러그인 등) → `peerDependencies`
- 버전이 lock에 맞아도 **resolve 경로가 갈라지면** 싱글톤이 깨지는 패키지 → `peerDependencies`

### `peerDependencies` (호스트 제공 · 싱글톤)
...
내부 `@watcha-authentic/*`는 **transitive install**, React는 **호스트 인스턴스 공유(싱글톤)** 를 위해 peer로 나누는 대표 패턴입니다.  
