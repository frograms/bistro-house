...
| 템플릿 | `platform` | 용도 |
| ------ | ---------- | ---- |
| `lib` | `node` | Node 전용 설정·유틸 (`prettier-config`, `eslint-config`) |
...
- `@watcha-authentic/*` 내부 패키지를 조합할 때 주로 사용 (`react-slider` → `react-motion` 등).
...
- 선택적 피어만 쓰는 preset (`eslint-config` + `peerDependenciesMeta.optional`)

**예:** `eslint-config` — 피어는 많지만 preset별로 필요한 것만 호스트가 설치. `peerDependenciesMeta.optional`로 선택 피어 표시.

### `devDependencies` (개발 전용)

- tsdown, typescript, eslint, `@types/*`, 로컬 테스트용 `react` 등.
- **publish tarball에 runtime으로 포함되지 않음** (소비자 install 대상 아님).
- 모노레po 안에서 lint/typecheck/build 돌릴 때만 사용.
...
| ESLint·PostCSS 플러그인 (호스트 toolchain) | `peerDependencies` (+ 필요 시 optional) |
| `@watcha-authentic/*` 내부 조합 (항상 함께 쓰는 하위 라이브러리) | `dependencies` |
| tsdown, eslint, prettier, `@types/*` | `devDependencies` |
| CLI 전용 (`commander`, `fs-extra`) | `dependencies` (실행 시 필요) |
...
템플릿: `packages/common-cli/project-resource/package-template/{lib,react}/tsdown.config.mts`
...
### common-cli (예외: multi-entry CLI)

`packages/common-cli/tsdown.config.mts` — `unbundle: true`, `entry: src/**/*.ts`, `platform: node`

- CLI는 파일 단위 CJS 산출 + `bin` 진입점.
- `commander`, `fs-extra`는 **`dependencies`** — dist에서 `require("commander")` 형태로 external.
- 템플릿(`lib`/`react`)과 설정을 **맞출 필요 없음**. CLI 전용 패턴.

## 워크스페이스 내부 vs npm 소비자

| 구분 | 모노repo (`workspace:*`) | npm 소비자 |
| ---- | ------------------------ | ---------- |
...
| eslint-config 피어 | `devDependencies`에 전체 (로컬 lint) | README 표 + `pnpm add -D` |

모노repo에서 `devDependencies`에 react·eslint를 넣는 것은 **로컬 validate**용이고, npm 사용자에게는 **peer + README**로 안내합니다.
...
| 패키지 | 참고 포인트 |
| ------ | ----------- |
| `react-slider` | internal `dependencies` + React `peerDependencies` |
