...
| `package.json` `bin`이 있는 CLI의 실행 시 의존성                 | `dependencies` (런타임에 필요)          |
...
대부분의 `lib` / `react` 패키지는 **단일 entry** 패턴입니다. `package.json`에 **`bin` 필드가 있으면** 아래 §「`bin` 패키지」처럼 **별도 빌드 설정**을 씁니다.
...
`package.json`에 `bin`이 있는 CLI·실행형 패키지는 **lib/react 단일 entry 템플릿과 맞출 필요가 없습니다.**

- tsdown: 보통 `unbundle: true`, `platform: "node"`, `bin` 진입 파일을 `entry`로 지정
- 산출: 파일 단위 CJS + `bin`이 가리키는 dist 경로
- CLI 실행에 필요한 패키지는 **`dependencies`** — dist에서 `require("…")` 형태로 **external** 유지

구체 설정은 해당 패키지의 `tsdown.config.mts`·`package.json` `bin`을 기준으로 확인합니다.
...
| `bin` CLI 패키지 | CLI 런타임 `dependencies` | 없음 (또는 호스트 toolchain) | `unbundle` 등 별도 설정 (위 §) |
