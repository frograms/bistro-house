...
`patch/**` 브랜치 push 시 [publish.yml](.github/workflows/publish.yml)의 `publish-patch` job이 `publish-patch.sh`로 변경 패키지를 `{version}-patch.{n}` 형태로 배포합니다 (dist-tag: `patch`, auth: `npm-oidc`). git tag(`@watcha-authentic/<pkg>@<version>`)·CHANGELOG 커밋·GitHub Release(`generate-notes`)까지 워크플로에서 처리합니다.
