...
로컬 카나리 배포는 `pnpm publish:canary`, patch 배포는 `bash ./project-attachment/script/publish-patch.sh` (각각 `npm login`).

### patch 배포 (구 정식 라인)

`patch/**` 브랜치 push 시 [publish.yml](.github/workflows/publish.yml)의 `publish-patch` job이 변경 패키지를 `{version}-patch.{n}` 형태로 배포합니다 (dist-tag: `patch`). Lerna와 동일한 형식의 git tag(`@watcha-authentic/<pkg>@<version>`)를 붙입니다.

```bash
# 로컬 (auth: login)
npm login
bash ./project-attachment/script/publish-patch.sh react-slider

# CI (auth: npm-oidc)
bash ./project-attachment/script/publish-patch.sh <package> npm-oidc
```
